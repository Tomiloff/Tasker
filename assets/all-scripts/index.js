$(function () {
   // ==Работа с header==
   // Открытие аккордеона в хедере
   $("#header-btn-desktop").click(function () {
      $(".header-list").slideToggle();
   })


   // Обработка клика вне аккордиона .header-list
   ...
   $("main").on("click", ".deleteSection", function () {
      let sampleMainSection = $(this).parent(".main-section");
      sampleMainSection.next(".main-section-tab").remove();
      sampleMainSection.remove();
   });


   // Обработка клика по кнопке .editSection для изменения названия раздела для tasks
   $("main").on("click", ".editSection", function (e) {
      if ($(this).parent(".main-section").children("#mainSectionName").length == 1) {
         e.preventDefault();
      }
      else {
         $(this).parent(".main-section").append("<input type='text' id='mainSectionName' maxlength='18'></input>").css({ "padding": "42px" });
         $(this).next().remove();
         $(this).parent().children("#addName").css({
            "display": "block"
         });
      }
   })
   ...
   $("main").on("click", "#addName", function () {
      ...
      if (newBlockName == "Важное") {
         $(this).parent(".main-section").append("<h2>" + newBlockName + "</h2>").css({ "padding": "43px" }).removeClass().addClass("main-section flex important-section").children("#mainSectionName").remove();
         $(this).parent().children("#addName").css({
            "display": "none"
         });
      }
      else if (newBlockName == "Работа") {
         $(this).parent(".main-section").append("<h2>" + newBlockName + "</h2>").css({ "padding": "43px" }).removeClass().addClass("main-section flex work-section").children("#mainSectionName").remove();
         $(this).parent().children("#addName").css({
            "display": "none"
         });
      }
      else if (newBlockName == "Личное") {
         $(this).parent(".main-section").append("<h2>" + newBlockName + "</h2>").css({ "padding": "43px" }).removeClass().addClass("main-section flex personal-section").children("#mainSectionName").remove();
         $(this).parent().children("#addName").css({
            "display": "none"
         });
      }
      else if (newBlockName == "Идеи") {
         $(this).parent(".main-section").append("<h2>" + newBlockName + "</h2>").css({ "padding": "43px" }).removeClass().addClass("main-section flex ideas-section").children("#mainSectionName").remove();
         $(this).parent().children("#addName").css({
            "display": "none"
         });
      }
      else {
         $(this).parent(".main-section").append("<h2>" + newBlockName + "</h2>").css({ "padding": "43px" }).removeClass().addClass("main-section flex").children("#mainSectionName").remove();
         $(this).parent().children("#addName").css({
            "display": "none"
         });
      }
   ...
   //Обработка клика по .main-section для открытия tab и начала работы с tasks, также меняме фон и заголовок у tab
   $("main").on("click", ".main-section", function (e) {
      let tabBackground = $(this).css("background-color");
      let tabTitle = $(this).children("h2").text();
      if ($(this).has(e.target).length > 0) {
         e.preventDefault();
      }
      else if ($("main").children(".main-section").length == $("main").children(".main-section-tab").length) {
         $(this).next(".main-section-tab").css({
            "display": "block",
         ...
         $(".main-section-tab #tab-header-title").text(tabTitle);
      }
      else {
         $(this).after("<div class=\"main-section-tab\"><div class=\"section-tab-header flex\"><button class=\"tab-header-btnExit\"><svg width=\"27\" height=\"24\" viewBox=\"0 0 21 18\" fill=\"none\"><path fill-rule=\"evenodd\" clip-rule=\"evenodd\" d=\"M14.6448 10.3589L19.0562 10.3657C19.607 10.3666 19.7349 10.0506 19.3442 9.65992L10.9777 1.29338C10.5847 0.900364 9.95284 0.903479 9.56121 1.29511L1.22862 9.62769C0.832895 10.0234 0.965192 10.3375 1.51496 10.3384L5.64476 10.3448V14.5247C5.64476 15.6281 6.53902 16.5243 7.64215 16.5243H12.6474C13.7489 16.5243 14.6448 15.629 14.6448 14.5247V10.3589Z\" stroke=\"black\" stroke-width=\"1.3\" /></svg></button><h2 id='tab-header-title'>" + tabTitle + "</h2></div><div class='tab-header-line'></div><button class=\"section-tab-btn flex\"><svg width=\"18\" height=\"18\" viewBox=\"0 0 18 18\" fill=\"none\"><path fill-rule=\"evenodd\" clip-rule=\"evenodd\" d=\"M10 6C10 7.10457 10.8954 8 12 8H17C17.5523 8 18 8.44771 18 9C18 9.55229 17.5523 10 17 10H12C10.8954 10 10 10.8954 10 12V17C10 17.5523 9.55229 18 9 18C8.44771 18 8 17.5523 8 17V12C8 10.8954 7.10457 10 6 10H1C0.447715 10 0 9.55229 0 9C0 8.44771 0.447715 8 1 8H6C7.10457 8 8 7.10457 8 6V1C8 0.447715 8.44771 0 9 0C9.55229 0 10 0.447715 10 1V6Z\" fill=\"#fff\" /></svg></button></div>");
         $("body").css({
            "overflow": "hidden"
         });
         let tabBackground = $(this).css("background-color");
         $(".main-section-tab").css("background-color", tabBackground);
      }
   });
